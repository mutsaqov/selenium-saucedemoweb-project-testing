...
├── logs/                     # [NEW] Stores execution log files (*.log)
├── pages/
...
│   └── product_detail_page.py# Locators & Actions for Product Details
├── Report_Test/              # HTML Test Reports generated by pytest
├── screenshots/              # Auto-captured screenshots on FAILURE
├── tests/
│   ├── base_test.py          # Setup Driver, Logging Config, & Teardown
│   ├── test_login_page.py    # Login & Logout Scenarios
│   ├── test_inventory.py     # Inventory & Sidebar Scenarios
│   ├── test_cart.py          # Cart Logic Scenarios
│   ├── test_checkout.py      # Checkout Flow & Math Validation
│   └── product_detail_test.py# PDP Logic Scenarios
└── README.md
```
...
This project has migrated from basic `print()` statements to a robust. This ensures that every test execution is traceable, auditable, and easier to debug.
